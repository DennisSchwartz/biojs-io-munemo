...
var Munemo = function ( input ) {
    ...
    network.prototype.vertexCount = function () {
        return network.nodelayers.length;
    };

    network.prototype.edgeCount = function () {
        return network.edges.length;
    };

    network.prototype.calcVertexDegrees = function (step, done) {
        ...
        for ( var i = 0; i < nl.length; i++ ) {
            var update = network.elements[nl[i].data.id];
            nl[i] = update;
            avg += update.degree;
        }
        avg = avg / nl.length;
    ...
    network.prototype.calcDegreeOfMultiplexity = function () {
        /*
