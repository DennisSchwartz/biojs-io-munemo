...
var Munemo = function ( input ) {
    ...
    var createMultiplexAdjacencyArray = function () {
        ...
        network.edges.forEach(function (e) {
            ...
            A[alpha] = mathjs.subset(A_alpha, mathjs.index(i, j), 1);
        });
    ...
    var calcVertexDegrees = function () {
        ...
        if (!network.elements['n' + network.nodes[0].data.id].degree) {
        ...
        for ( var i = 0; i < A.length; i++ ) {
            A[i].forEach(function (value, index, matrix) {
                ...
                if (src.data.degree.length < i) src.data.degree.push(0);
        ...
        //        math.subset(network.adjacencyMatrix, math.index(i, k));
        //    }
        //}
    };
