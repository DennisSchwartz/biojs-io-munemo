...
var Munemo = function ( input ) {
    ...
    var calcVertexDegrees = function () {
        ...
        for ( i = 0; i < network.nodes.length; i++ ) {
            network.nodes[i].data.degree = Array.apply(null, Array(5)).map(Number.prototype.valueOf,0);
            process.stdout.write("Initialising nodes: " +
                ( (i / network.nodes.length) * 100 ).toFixed(2) + "% \r");
        }

        var max = A.length * Math.pow(network.nodes.length, 2);
        var counter = 0;
        for ( i = 0; i < A.length; i++ ) {
            A[i].forEach(function (value, index, matrix) {
                network.nodes[index[0]].data.degree[index] += value;
                network.nodes[index[1]].data.degree[index] += value;
                counter++;
