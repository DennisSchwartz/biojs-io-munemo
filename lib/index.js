...
var Munemo = function ( input ) {
    ...
    var createMultiplexAdjacencyArray = function () {
        ...
        o = o.map(function (value, index, matrix) {
            return value > 1 ? 1 : 0;
        });
    ...
    var layerStrength = function (l) {
        var strength = mathjs.zeros(network.layers.length);
        network.nodes.forEach(function (n) {
            strength = mathjs.add(strength, n.data.degree);
        });
        if (l) {
            var layerIndex = R.map(function (i) { return i.data['id'] }, network.layers).indexOf('l' + l);
            console.log("layerIndex: " + layerIndex);
            console.log("Strength: " + strength, strength.subset(mathjs.index(layerIndex)));
            return strength.subset(mathjs.index(layerIndex));
        } else {
            return strength;
        }
    };
